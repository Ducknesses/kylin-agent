    ...
    def __init__(self, client: MCPClient | None = None):
        ...
        async def execute(
            ...
            if tool_name not in get_tool_names():
                ...
                return {"ok": False, "result": None, "error": f"未知工具: {tool_name}"}
            ...
            if tool_name == "cmd_exec" and "command" in arguments:
                ...
                if not perm["allowed"]:
                    ...
                    return {"ok": False, "result": None, "error": f"权限不足: {perm['reason']}"}

            # 3. 调用 MCP Client
            logger.info(f"[Executor] 执行工具 {tool_name}，参数字段: {list(arguments.keys())}")
