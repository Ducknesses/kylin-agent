...
def is_high_risk_command(user_input: str) -> bool:
    ...
    # 单独处理 curl ... | sh 和 wget ... | sh
    lower = user_input.lower()
    for p in high_risk_patterns:
        if p in lower:
            return True
    if "curl " in lower and "|" in lower and "sh" in lower:
        return True
    if "wget " in lower and "|" in lower and "sh" in lower:
        return True
    return False


def is_medium_risk_command(user_input: str) -> bool:
    """检查输入是否为中危操作（需要确认）"""
    ...
    lower = user_input.lower()
    return any(k in lower for k in medium_risk_keywords)
