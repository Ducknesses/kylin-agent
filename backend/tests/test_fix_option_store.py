...
使用 fakeredis + 可控 fake clock，不依赖真实 Redis 或时间等待。
"""
...
# 模块级共享 FakeRedis client，避免每个测试重复创建
_shared_client: "fakeredis.FakeRedis | None" = None


def _get_shared_client():
    """获取或创建模块级共享 FakeRedis client"""
...
def _make_store(ttl_seconds: int = 86400, clock=None):
    """创建 fakeredis 支持的 FixOptionStore"""
    client = _get_shared_client()
    storage = RedisStorage(client=client)
    return FixOptionStore(ttl_seconds=ttl_seconds, clock=clock, storage=storage)
...
class TestSaveAndGet:
    ...
    def test_redis_restore_read(self):
        """Redis 恢复读取：写入后使用同一 Redis 创建新 store 实例，确认可恢复"""
        client = _get_shared_client()

        store1 = FixOptionStore(ttl_seconds=86400, storage=RedisStorage(client=client))
        store1.save_options("s2", "t2", [_fix_option("fix_a1b2c3d4", title="Redis恢复")])

        # 模拟"重启"：使用同一个 fakeredis client 创建新 store
        store2 = FixOptionStore(ttl_seconds=86400, storage=RedisStorage(client=client))
        stored = store2.get_option("s2", "fix_a1b2c3d4")
        assert stored is not None
        assert stored.option.title == "Redis恢复"
...
class TestIsolation:
    ...
    def test_duplicate_after_expired_allowed(self):
        ...
        store = _make_store(ttl_seconds=60, clock=_fixed_clock(t0))
        store.save_options("s1", "t1", [_fix_option("fix_0e9f1e8e")])
        # 快进到过期
...
class TestInputValidation:
    ...
    def test_ttl_zero_rejected(self):
        with pytest.raises(ValueError, match="ttl_seconds"):
            FixOptionStore(ttl_seconds=0)  # 无 Redis 也会在 TTL 校验时失败
