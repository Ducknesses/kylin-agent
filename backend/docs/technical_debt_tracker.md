...
### TD-07：MCP `result.blocked=true` 场景未覆盖
...
**问题描述**：
当前 Mock 工具调用结果均为成功数据，未覆盖 MCP Server 返回 `blocked=true` 时的异常处理与前端提示。

**验收标准**：
- [ ] `executor.py` 或 `client.py` 能正确处理 MCP Server 返回的 `blocked=true` 响应
- [ ] 前端收到对应的 `error` 或 `tool_call`（带 `blocked` 标记）消息并展示提示
- [ ] 测试用例覆盖 blocked 场景
...
### TD-10：`GET /api/audit/logs` legacy 路由仍存在
...
**问题描述**：
正式规范使用 `GET /api/audit`，但 `/api/audit/logs` 仍作为兼容路由保留。

**验收标准**：
- [ ] 前端审计页全部通过 `GET /api/audit` 调用
- [ ] `/api/audit/logs` 路由删除或返回 301 重定向
- [ ] 无旧调用方依赖此路径

**关联文件**：
- `backend/app/api/audit.py`
- `frontend/src/components/AuditTable.vue`
...
### TD-12：`audit.db` 从 Git 跟踪移除需手动执行
...
**问题描述**：
`backend/data/audit.db` 已被 `.gitignore` 排除，但若之前已 `git add` 跟踪，需手动执行 `git rm --cached backend/data/audit.db` 才能从 Git 索引中移除。
...
| ~~TD-15-old~~ | Day 1 | `.gitignore` 末尾缺换行符 | 补充换行 |
...
| **P2** | 4 | TD-03（ConnectionManager 拆分）、TD-06（FixOption）、TD-07（blocked 场景）、TD-08（多 worker 状态）、TD-09（MCP 监控路由） |
| **P3** | 4 | TD-04（死代码）、TD-10（legacy 路由）、TD-11（时间过滤验证）、TD-12（git rm audit.db） |
